...
Checkbox object: a genuinely standalone class, pulled out of the old
objects/widget/widget.c grab-bag the same way LED and Button already
were - a checkbox is a checkbox, not a generic Widget parameterized
down to one. See objects/led/led.c's doc comment for the reasoning.
...
"SomeProp", Checkbox1, "In") is enough, since every property fans out
...
{
	DebugPrint ( "Checkbox handling a message.", __FILE__, __LINE__, OBJMSGHANDLING);
	return rtrn_handled;
}
...
int Checkbox_OnIn(NodeObj instance, MsgId message, NodeObj data)
{
...
int Checkbox_OnEnable(NodeObj instance, MsgId message, NodeObj data)
{
	InstanceData *local = (InstanceData *)GetPropLong(instance, "local");
...
int Checkbox_Activate(NodeObj instance, MsgId message, NodeObj data)
{
	InstanceData *local = (InstanceData *)GetPropLong(instance, "local");
...
int InstanceStart(NodeObj class, MsgId message, NodeObj data)
{
	...
	SetPropLong(port, "OnMsg", (long)Checkbox_OnIn);
...
int ClassStart(NodeObj library, MsgId message, NodeObj data)
{
	...
	PublishShow(ClassSelf, PROP_CHECKBOX, show_web_js, show_web_css);

	PublishProp(ClassSelf, "Value", PROP_CHECKBOX, "0");
...
void _init()
{
	...
	SetName(temp, "Checkbox");
	SetPropStr(temp, "Company", "GrokThink");
	SetPropStr(temp, "UUID", "8da17004-242c-4f21-a77e-6a823a52c741");
