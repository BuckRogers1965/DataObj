...
Slider object: a genuinely standalone class, pulled out of the old
...
"SomeProp", Slider1, "In") is enough, since every property fans out
...
{
	DebugPrint ( "Slider handling a message.", __FILE__, __LINE__, OBJMSGHANDLING);
	return rtrn_handled;
}
...
int Slider_OnIn(NodeObj instance, MsgId message, NodeObj data)
{
...
int Slider_OnEnable(NodeObj instance, MsgId message, NodeObj data)
{
	InstanceData *local = (InstanceData *)GetPropLong(instance, "local");
...
int Slider_Activate(NodeObj instance, MsgId message, NodeObj data)
{
	InstanceData *local = (InstanceData *)GetPropLong(instance, "local");
...
int InstanceStart(NodeObj class, MsgId message, NodeObj data)
{
	...
	SetPropLong(port, "OnMsg", (long)Slider_OnIn);
...
int ClassStart(NodeObj library, MsgId message, NodeObj data)
{
	...
	PublishShow(ClassSelf, PROP_SLIDER, show_web_js, show_web_css);

	PublishProp(ClassSelf, "Value", PROP_SLIDER, "0");
...
void _init()
{
	...
	SetPropStr(temp, "UUID", "8da17004-242c-4f21-a77e-6a823a52c743");
