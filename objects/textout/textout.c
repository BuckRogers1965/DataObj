...
TextOut object: a genuinely standalone class, pulled out of the old
...
TextOut1, "In") is enough, since every property fans out to whatever's
...
{
	DebugPrint ( "TextOut handling a message.", __FILE__, __LINE__, OBJMSGHANDLING);
	return rtrn_handled;
}
...
int TextOut_OnIn(NodeObj instance, MsgId message, NodeObj data)
{
...
int TextOut_OnEnable(NodeObj instance, MsgId message, NodeObj data)
{
	InstanceData *local = (InstanceData *)GetPropLong(instance, "local");
...
int TextOut_Activate(NodeObj instance, MsgId message, NodeObj data)
{
	InstanceData *local = (InstanceData *)GetPropLong(instance, "local");
...
int InstanceStart(NodeObj class, MsgId message, NodeObj data)
{
	...
	SetPropLong(port, "OnMsg", (long)TextOut_OnIn);

	SetPropStr(instance, "Enable", "1");
	port = GetPropNode(instance, "Enable");
	SetPropLong(port, "OnMsg", (long)TextOut_OnEnable);
...
int ClassStart(NodeObj library, MsgId message, NodeObj data)
{
	...
	PublishShow(ClassSelf, PROP_TEXTOUT, show_web_js, show_web_css);

	PublishProp(ClassSelf, "Value", PROP_TEXTOUT, "");
...
void _init()
{
	...
	SetName(temp, "TextOut");
	SetPropStr(temp, "Company", "GrokThink");
	SetPropStr(temp, "UUID", "8da17004-242c-4f21-a77e-6a823a52c746");
