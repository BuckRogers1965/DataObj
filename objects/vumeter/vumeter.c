...
VUMeter object: a genuinely standalone class, pulled out of the old
objects/widget/widget.c grab-bag the same way LED and Button already
were. See objects/led/led.c's doc comment for the reasoning - "an LED
is an LED, a VU meter is a VU meter" applies here most literally of
all: this was the exact example used to argue against a shared generic
readout treatment, so it gets its own file first among the display-only
widgets.
...
VUMeter1, "In") is enough, since every property fans out to whatever's
...
{
	DebugPrint ( "VUMeter handling a message.", __FILE__, __LINE__, OBJMSGHANDLING);
	return rtrn_handled;
}
...
int VUMeter_OnIn(NodeObj instance, MsgId message, NodeObj data)
{
...
int VUMeter_OnEnable(NodeObj instance, MsgId message, NodeObj data)
{
	InstanceData *local = (InstanceData *)GetPropLong(instance, "local");
...
int VUMeter_Activate(NodeObj instance, MsgId message, NodeObj data)
{
	InstanceData *local = (InstanceData *)GetPropLong(instance, "local");
...
int ClassStart(NodeObj library, MsgId message, NodeObj data)
{
	...
	PublishProp(ClassSelf, "Value",  "data", PROP_VUMETER, "0");
...
void _init()
{
	...
	SetName(temp, "VUMeter");
	SetPropStr(temp, "Company", "GrokThink");
	SetPropStr(temp, "UUID", "8da17004-242c-4f21-a77e-6a823a52c747");
