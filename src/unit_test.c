...
/* ---- copied verbatim from object.c ---- */
/* NOT COMPILED - UT_InterfaceTest: uses KnownClasses, a static array in object.c.
   #if 0 rather than a block comment because the body contains comments. */
#if 0
...
#endif		/* UT_InterfaceTest */

/* ---- copied verbatim from object.c ---- */
/* NOT COMPILED - UT_SkinTest: uses KnownClasses, a static array in object.c.
   #if 0 rather than a block comment because the body contains comments. */
#if 0
void UT_SkinTest(){
...
int main ( int argc, char* argv[] ){
	...
	/* ------------------------------------------------------------------
	   InstallObjects() IS COMMENTED OUT HERE - and it is worth arguing about
	   whether it belongs in a host at all.

	   Whether the framework CAN load modules is mechanism. Which modules, and
	   from where, is policy: the current call scans down from the working
	   directory to depth 8 and dlopens everything ending in .object, which is
	   a decision the host makes on behalf of every app that ever links the
	   library. The deployment story already says the opposite - shipping a
	   different product means shipping different objects and a different flow,
	   never a different binary - and that only holds if the APP picks its own
	   object path. Two apps on one machine could then have different palettes,
	   and an embedder could load nothing and hand-build what it wants.

	   The same argument applies harder to what comes after it: BuildPalette()
	   instantiates every registered class, and BuildChrome() builds the topbar
	   menus. Those are app furniture, and building them here means the mere
	   act of starting the host creates live instances - each of which arms its
	   object's tasks. For this harness that is the whole problem: with modules
	   loaded, the task list is never empty at the end, and a "leftover task"
	   says nothing about the core code under test.

	   So for now: no modules, no instances, and a leftover task means a CORE
	   test left one. The three tests that need registered classes (FlowTest
	   needs Reader/Writer/Pulse/Out, InterfaceTest and SkinTest walk
	   KnownClasses) already skip themselves when the classes are absent - and
	   a core test that needs a loadable module arguably is not a core test.
	   ------------------------------------------------------------------ */
	/* InstallObjects(); */
...

	{
		...
		printf("%s: %d task%s left on the list\n", left ? "FAIL" : "ok",
			   left, left == 1 ? "" : "s");
		exit(left > 254 ? 254 : left);	/* the return code IS the count */
	}
